...
class BestNewAppAdapter(var list: List<MoreGame>, var onClickNewGame: (item: MoreGame) -> Unit) :
    RecyclerView.Adapter<BestNewAppAdapter.BestNewAppViewHolder>() {

    class BestNewAppViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BestNewAppViewHolder {
        val view =
            LayoutInflater.from(parent.context).inflate(R.layout.more_game_row, parent, false)
        return BestNewAppViewHolder(view)

    }

    override fun onBindViewHolder(holder: BestNewAppViewHolder, position: Int) {
