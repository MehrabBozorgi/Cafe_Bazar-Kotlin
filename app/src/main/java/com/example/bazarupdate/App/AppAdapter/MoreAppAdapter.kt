...
class MoreAppAdapter(var list: List<MoreGame>, var onClickNewGame: (item: MoreGame) -> Unit) :
    RecyclerView.Adapter<MoreAppAdapter.MoreAppViewHolder>() {

    class MoreAppViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MoreAppViewHolder {
        var view =
            LayoutInflater.from(parent.context).inflate(R.layout.more_game_row, parent, false)
        return MoreAppViewHolder(view)

    }

    override fun onBindViewHolder(holder: MoreAppViewHolder, position: Int) {
