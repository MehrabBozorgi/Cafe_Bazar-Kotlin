...
class UpdateAppAdapter(var list: List<MoreGame>, var onClickNewGame: (item: MoreGame) -> Unit) :
    RecyclerView.Adapter<UpdateAppAdapter.UpdateAppViewHolder>() {

    class UpdateAppViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): UpdateAppViewHolder {
        var view =
            LayoutInflater.from(parent.context).inflate(R.layout.more_game_row, parent, false)
        return UpdateAppViewHolder(view)

    }

    override fun onBindViewHolder(holder: UpdateAppViewHolder, position: Int) {
