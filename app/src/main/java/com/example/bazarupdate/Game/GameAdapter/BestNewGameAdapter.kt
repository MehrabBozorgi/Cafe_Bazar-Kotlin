...
class BestNewGameAdapter(var list: List<MoreGame>, var onClickBestNew: (item: MoreGame) -> Unit) :
    RecyclerView.Adapter<BestNewGameAdapter.BestNewViewHolder>() {

    class BestNewViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BestNewViewHolder {
        var view =
            LayoutInflater.from(parent.context).inflate(R.layout.more_game_row, parent, false)
        return BestNewViewHolder(view)

    }

    override fun onBindViewHolder(holder: BestNewViewHolder, position: Int) {
