...
class AnimatAdapter (var list: List<Movie>, var onItemClick: (item: Movie) -> Unit) :
    RecyclerView.Adapter<AnimatAdapter.AnimatViewHolder>() {

    class AnimatViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AnimatViewHolder {

        var view =
            LayoutInflater.from(parent.context).inflate(R.layout.film_iran_row, parent, false)
        return AnimatViewHolder(
            view
        )

    }

    override fun onBindViewHolder(holder: AnimatViewHolder, position: Int) {
