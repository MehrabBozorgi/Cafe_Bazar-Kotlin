...
class UpdateFilmAdapter(var list: List<Movie>, var onItemClick: (item: Movie) -> Unit) :
    RecyclerView.Adapter<UpdateFilmAdapter.UpdateFilmViewHolder>() {

    class UpdateFilmViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): UpdateFilmViewHolder {

        var view =
            LayoutInflater.from(parent.context).inflate(R.layout.film_iran_row, parent, false)
        return UpdateFilmViewHolder(
            view
        )

    }

    override fun onBindViewHolder(holder: UpdateFilmViewHolder, position: Int) {
